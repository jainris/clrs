...
class PriorityQueue_atv2(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
        ...
        self.nb_heads = nb_heads
        self.aggregation_technique = aggregation_technique
        self.mid_size = embedding_size // nb_heads
    ...
    def __call__(
        ...
        w_1 = hk.Linear(self.nb_heads * self.mid_size)
        w_2 = hk.Linear(self.nb_heads * self.mid_size)

        a_heads = []
        for _ in range(self.nb_heads):
            a_heads.append(hk.Linear(1))

        pre_att_1 = w_1(z)  # [B, N, H * K]
        pre_att_2 = w_2(prev_state.memory_values)  # [B, S, H * K]

        pre_att_1 = jnp.reshape(
            pre_att_1, (batch_size, nb_nodes, self.nb_heads, self.mid_size, 1)
        )  # [B, N, H, K, 1]
        pre_att_2 = jnp.reshape(
            pre_att_2, (batch_size, self._memory_size, self.nb_heads, self.mid_size, 1)
        )  # [B, S, H, K, 1]

        # [B, H, N, 1, K] + [B, H, 1, S, K]  = [B, H, N, S, K]
        pre_att = jnp.transpose(pre_att_1, (0, 2, 1, 4, 3)) + jnp.transpose(
            pre_att_2, (0, 2, 4, 1, 3)
        )

        vals = jax.nn.leaky_relu(pre_att)
        logit_heads = []
        for head in range(self.nb_heads):
            logit_heads.append(
                jnp.squeeze(a_heads[head](vals[:, head]), axis=-1)
            )  # [B, N, S]
        logits = jnp.stack(logit_heads, axis=1)  # [B, H, N, S]
        ...
        coefs = jax.nn.softmax(logits + bias_mat, axis=-1)  # [B, H, N, S]
        # Projecting from the different heads into a single importance value
        coefs = jnp.transpose(coefs, (0, 2, 3, 1))  # [B, N, S, H]
        if self.nb_heads > 1:
            # Multi-head
...
class PriorityQueue_ProperHeads(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
    ...
    def __call__(
        ...
        if self.aggregation_technique == "max":
            ...
            # val = jnp.tile(prev_state.memory_values, (batch_size, 1, 1, 1)) # [B, S, F']
            # print("val shape: {}".format(val.shape))
            output = jnp.matmul(coefs, val)  # [B, H, N, F']
        else:
            raise ValueError(
                "Unknown memory aggregation technique " + self.aggregation_technique
            )
...
class PriorityQueueV2_ProperHeads(MemoryModule):
    ...
    def __call__(
        ...
        output_coefs = pop_proportions * jnp.expand_dims(
            pop_given, axis=[1, 2]
        )  # [B, H, N, S]
...
class PriorityQueueV2_single_value(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
    ...
    def __call__(
        ...
        # Aggregating the attention values
        coefs = jnp.sum(coefs, axis=1, keepdims=True)  # [B, 1, S]
        coefs = jax.nn.softmax(coefs, axis=-1)  # [B, 1, S]
        coefs = jnp.tile(coefs, (1, nb_nodes, 1))  # [B, N, S]
...
class PriorityQueueV2_Sigmoid(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
    ...
    def __call__(
        ...
        push_strengths = push_proj(z)  # [B, N, 1]
        push_strengths = jnp.sum(push_strengths, axis=1)  # [B, 1]
        push_strengths = jax.nn.sigmoid(push_strengths)  # [B, 1]

        pop_strengths = pop_proj(z)  # [B, N, 1]
...
class PriorityQueueV2_Sigmoid_atv2(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
...
class PriorityQueueState_Hardcoded(MemoryState):
    iteration_mask: _Array


class PriorityQueue_HardCoded(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
    ...
    def initial_state(
        self, batch_size: int, nb_nodes: int, **kwargs
    ) -> PriorityQueueState_Hardcoded:
        iteration_mask = jax.nn.one_hot(
            [0] * batch_size,
            num_classes=nb_nodes,
            dtype=jnp.float32,
        )  # [B, N]
        return PriorityQueueState_Hardcoded(iteration_mask=iteration_mask)
    ...
    def __call__(
        ...
        us: _Array,
        us_pi: _Array,
        prev_state: PriorityQueueState_Hardcoded,
        **kwargs
    ) -> Tuple[_Array, PriorityQueueState_Hardcoded]:
        # z.shape: [B, N, F]
        batch_size, nb_nodes, nb_z_fts = z.shape
        if prev_state is None:
        ...
        output_proj = hk.Linear(output_size=self._output_size)
        node_proj = hk.Linear(output_size=self._embedding_size)
        ...
        u_pi = us_pi * jnp.expand_dims(prev_state.iteration_mask, axis=-1)  # [B, N, N]
        u_pi = jnp.sum(u_pi, axis=1)  # [B, N]
        ...
        return output, PriorityQueueState_Hardcoded(iteration_mask=new_iteration_mask)


class PriorityQueue_HardCoded_delay(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
...
class PriorityQueue_HardCoded_2(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
...
class PriorityQueue_HardCoded_2_delay(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
...
class PriorityQueue_HardCoded_2_return_u(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
    ...
    def __call__(
        ...
        prev_state: PriorityQueueState_Hardcoded,
        **kwargs
    ) -> Tuple[_Array, _Array, PriorityQueueState_Hardcoded]:
        # z.shape: [B, N, F]
        batch_size, nb_nodes, nb_z_fts = z.shape
        if prev_state is None:
            prev_state = self.initial_state(batch_size=batch_size, nb_nodes=nb_nodes)
...
class PriorityQueue_HardCoded_3_return_u(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
    ...
    def __call__(
        ...
        **kwargs
        ...
        return output, u, PriorityQueueState_Hardcoded_2(
            iteration_mask=new_iteration_mask, pushed_values=new_push_values
        )


class PriorityQueue_HardCoded_3_return_u_delay(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
...
class PriorityQueue_HardCoded_3_delay(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
