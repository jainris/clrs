...
class PriorityQueueV2_Sigmoid_ProperHeads(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
...
class PriorityQueueV2_Sigmoid_SingleValue(MemoryModule):
    def __init__(
        self,
        output_size: int,
        embedding_size: int,
    ...
    def __call__(
        ...
        pop_strengths = pop_proj(z)  # [B, N, 1]
        pop_strengths = jnp.sum(pop_strengths, axis=1)  # [B, 1]
        pop_strengths = jax.nn.sigmoid(pop_strengths)  # [B, 1]
        node_wise_pop_strengths = jnp.expand_dims(pop_strengths, axis=1)  # [B, 1, 1]
...
def memory_factory(
    ...
    elif memory_module == "priority_queue_v2_sig_ph":
    ...
    elif memory_module == "priority_queue_v2_sig_sv":
        return PriorityQueueV2_Sigmoid_SingleValue(
            output_size=message_dimension
            if memory_module_args["direct_output"]
            else feature_dimension,
            embedding_size=feature_dimension
...
) -> Tuple[_Array, _Array, MemoryState, MemoryModule]:
