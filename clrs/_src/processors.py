...
from clrs._src.memory import PriorityQueue_HardCoded_delay
...
from clrs._src.memory import PriorityQueue_HardCoded_3_return_u_delay
...
class PGN_pq_hardcoded(Processor):
  ...
  def __call__(
    ...
    elif self.memory_module == 'priority_queue_hardcoded_delay':
      memory_module = PriorityQueue_HardCoded_delay(
        output_size=msgs.shape[-1] if self.memory_module_args['direct_output'] else z.shape[-1],
        embedding_size=z.shape[-1] if self.memory_module_args['embedding_size'] is None else self.memory_module_args['embedding_size'],
      )
    ...
    elif self.memory_module == 'priority_queue_hardcoded_3_return_u_delay':
      memory_module = PriorityQueue_HardCoded_3_return_u_delay(
        output_size=msgs.shape[-1] if self.memory_module_args['direct_output'] else z.shape[-1],
        embedding_size=z.shape[-1] if self.memory_module_args['embedding_size'] is None else self.memory_module_args['embedding_size'],
      )
    else:
      raise ValueError('Unexpected processor memory kind ' + self.memory_module)
    if self.memory_module in [
      'priority_queue_hardcoded_2_return_u',
      'priority_queue_hardcoded_3_return_u',
